-- Invite-only Auth login: link auth users to dealers and start a dealer session.
ALTER TABLE public.dealers
  ADD COLUMN IF NOT EXISTS email text,
  ADD COLUMN IF NOT EXISTS auth_user_id uuid;

CREATE UNIQUE INDEX IF NOT EXISTS dealers_email_lower_uidx
  ON public.dealers (lower(email))
  WHERE email IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS dealers_auth_user_id_uidx
...
REVOKE EXECUTE ON FUNCTION public.dealer_login(text, text, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.dealer_login(text, text, text) FROM anon;
REVOKE EXECUTE ON FUNCTION public.dealer_login(text, text, text) FROM authenticated;
